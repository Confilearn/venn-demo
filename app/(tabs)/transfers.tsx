...
import { beneficiaries, transactions, formatCurrency, formatDate } from '@/lib/mock-data';
...
export default function TransfersScreen() {
  ...
  const transferTx = transactions.filter(t => t.category === 'Transfer' || t.type === 'debit');
    ...
    <View style={[styles.screen, { backgroundColor: colors.background }]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: insets.top + 16 + webTop, paddingBottom: 100 + webBottom }}>
        <View style={styles.headerRow}>
          <Text style={[styles.pageTitle, { color: colors.text, fontFamily: 'DMSans_700Bold' }]}>Transfers</Text>
          <Pressable
          ...
          >
            ...
            <Text style={[styles.sendBtnText, { fontFamily: 'DMSans_600SemiBold' }]}>Send Money</Text>
          </Pressable>
        </View>

        <View style={[styles.card, { backgroundColor: colors.card }]}>
          <View style={styles.cardHeader}>
            <Text style={[styles.cardTitle, { color: colors.text, fontFamily: 'DMSans_600SemiBold' }]}>Beneficiaries</Text>
            <Pressable onPress={() => Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light)}>
              <Feather name="plus" size={20} color={colors.primary} />
            </Pressable>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.beneficiaryRow}>
            {beneficiaries.map(b => (
              <Pressable
                key={b.id}
                style={({ pressed }) => [styles.beneficiary, { opacity: pressed ? 0.7 : 1 }]}
                onPress={() => {
                  Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                  router.push('/send-money');
                }}
              >
                <View style={[styles.beneficiaryAvatar, { backgroundColor: colors.accentMuted }]}>
                  <Text style={[styles.beneficiaryInitials, { color: colors.accent, fontFamily: 'DMSans_700Bold' }]}>{b.avatar}</Text>
                </View>
                <Text style={[styles.beneficiaryName, { color: colors.textSecondary, fontFamily: 'DMSans_500Medium' }]} numberOfLines={1}>{b.name.split(' ')[0]}</Text>
              </Pressable>
            ))}
          </ScrollView>
        </View>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.text, fontFamily: 'DMSans_600SemiBold' }]}>Transfer History</Text>
                ...
                >
                  ...
                  <View style={styles.txInfo}>
                    ...
                    <View style={styles.txMeta}>
                      <Text style={[styles.txDate, { color: colors.textSecondary, fontFamily: 'DMSans_400Regular' }]}>{formatDate(tx.date)}</Text>
                      {tx.status !== 'completed' && (
...
const styles = StyleSheet.create({
  ...
  card: { marginHorizontal: 20, borderRadius: 18, padding: 20, marginBottom: 24 },
  cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  cardTitle: { fontSize: 16 },
  beneficiaryRow: { gap: 16 },
  beneficiary: { alignItems: 'center', width: 60 },
  beneficiaryAvatar: { width: 48, height: 48, borderRadius: 24, alignItems: 'center', justifyContent: 'center', marginBottom: 6 },
